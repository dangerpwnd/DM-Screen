...
class SubClassModel(Base):
    ...
    prof_assoc = Table('Subclass_has_Proficiencies', Base.metadata,
    ...
    feature_assoc = Table('Subclass_has_Features', Base.metadata,
        Column('feature_id', Integer, ForeignKey('Feature.id_feature'), primary_key=True),
        Column('subclass_id', Integer, ForeignKey('Subclass.id_subclass'), primary_key=True)
    )

    spell_assoc = Table('Subclass_has_Spells', Base.metadata,
    ...
    charclass = relationship('CharClassModel', back_populates='subclasses')
